...
export class PowerPlantsController {
  ...
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Number of results to return (max 100)' })
  ...
  async getTopPowerPlants(@Query() dto: GetTopPowerPlantsDto): Promise<PowerPlantResponseDto> {
    ...
    try {
      ...
      // Cache the result for 5 minutes (only if not empty)
  ...
  async getStates(): Promise<StatesResponseDto> {
    ...
    try {
      ...
      // Cache the result for 10 minutes (states don't change often, only if not empty)
      if (states.length > 0) {
